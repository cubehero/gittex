...
(function() {
  ...
  exports.nativeGit = function(cmd) {
    var arg, args, argv, callback, gitBinary, gitCmd, gitDir, isDebug, options, options_to_argv, spawnOptions, stderrBufs, stdoutBufs, workTree, _i, _j, _len, _len1, _result;
    ...
    argv = [];
    if (gitDir != null) {
      argv.push("--git-dir=" + gitDir);
    }
    if (workTree != null) {
      argv.push("--work-tree=" + workTree);
    }
    argv.push(cmd);
    argv = argv.concat(options_to_argv(options));
    argv = argv.concat(args);
    if (isDebug === true) {
      console.log(gitBinary, argv, spawnOptions);
    }
    ...
    gitCmd.on('exit', function(exitCode, signal) {
      var buf, err, gitErrMsg, stderrBuf, stdoutBuf, _k, _l, _len2, _len3;
      if (exitCode > 1) {
      ...
      stdoutBuf = "";
      for (_k = 0, _len2 = stdoutBufs.length; _k < _len2; _k++) {
        buf = stdoutBufs[_k];
        stdoutBuf = stdoutBuf.concat(buf.toString());
      }
      stderrBuf = "";
      for (_l = 0, _len3 = stderrBufs.length; _l < _len3; _l++) {
        buf = stderrBufs[_l];
